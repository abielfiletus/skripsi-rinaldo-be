...
class ClassService {
  ...
  async getAll(body) {
    ...
    if (body.form['name']) {
      where.push(`c.name iLike '%${body.form['name']}%'`)
    }

    if (body.form['user_id']) {
      where.push(`c.createdBy = '${body.form['user_id']}'`)
    }

    if (body.form['code']) {
      where.push(`c.code = '${body.form['code']}'`)
    }

    if (body.form['start']) {
      where.push(`c.start >= '${body.form['start']}'`)
    }

    if (body.form['end']) {
      where.push(`c.end <= '${body.form['end']}'`)
    }
