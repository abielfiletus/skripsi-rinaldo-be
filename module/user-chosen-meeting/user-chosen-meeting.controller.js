...
class UserChosenMeetingController {
  ...
  async registerUsulan(req, res, next) {
    try {
      const bulk = { chosen_date: req.body.chosen_date ? req.body.chosen_date : null }

      const check = await service.getOneByUserAndMeeting(req.body.user_id, req.body.usulan_meeting_id)

      if (!check) return outputParser.fail(res, 400, 'Validation Error', { id: 'Tidak ditemukan' })

      const data = await service.update(check['id'], bulk)
